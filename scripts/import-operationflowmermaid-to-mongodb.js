...
const COLLECTION_NAME = 'operation_flow_mermaid';

// 读取 JSON 文件
function loadOperationFlowMermaidData() {
  try {
    const filePath = path.join(__dirname, '../src/data/subflow/operationFlowMermaid.json');
    const fileContent = fs.readFileSync(filePath, 'utf8');
    return JSON.parse(fileContent);
  } catch (error) {
    console.error('读取 operationFlowMermaid.json 文件失败:', error);
    throw error;
  }
}

async function importOperationFlowMermaidData() {
  let client;
  ...
  try {
    ...
    // 读取数据
    console.log('正在读取 operationFlowMermaid.json 文件...');
    const mermaidData = loadOperationFlowMermaidData();
    ...
    const importData = {
      ...mermaidData,
      importedAt: new Date(),
      source: 'operationFlowMermaid.json',
      description: '运维流程 Mermaid 图表数据 - 包含设备维护、故障处理、技术支持等流程',
      flowType: 'operation',
      version: '1.0'
    };
...
if (require.main === module) {
  importOperationFlowMermaidData();
}

module.exports = { importOperationFlowMermaidData }; 