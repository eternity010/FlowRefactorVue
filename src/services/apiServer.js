...
// 获取火车装配人员列表及其任务匹配度统计
app.get('/api/topic03/train-assembly/users', async (req, res) => {
  try {
    const { sort_by, sort_order, page, page_size, model_run_batch } = req.query;
    ...
    const options = {
      ...
      page: parseInt(page) || 1,
      pageSize: parseInt(page_size) || 20,
      modelRunBatch: model_run_batch || 'TRAIN_ASSEMBLY_2025'
    };

    const result = await topic03Service.getTrainAssemblyUsers(options);
    sendResponse(res, result, '获取火车装配人员列表失败');
  } catch (error) {
    sendError(res, error);
  }
});
...
// 获取特定火车装配人员的详细任务匹配信息
app.get('/api/topic03/train-assembly/user/:userId', async (req, res) => {
  try {
    const { userId } = req.params;
    const { model_run_batch, sort_by, sort_order } = req.query;
    ...
    const result = await topic03Service.getTrainAssemblyUserDetail(parseInt(userId), options);
    sendResponse(res, result, `获取用户 ${userId} 的详细匹配信息失败`);
  } catch (error) {
    sendError(res, error);
  }
});
...
// 获取火车装配任务匹配统计
app.get('/api/topic03/train-assembly/statistics', async (req, res) => {
  try {
    const { model_run_batch } = req.query;
    ...
    const result = await topic03Service.getTrainAssemblyStatistics(options);
    sendResponse(res, result, '获取火车装配统计数据失败');
  } catch (error) {
    sendError(res, error);
  }
});
...
// 搜索火车装配人员匹配记录
app.get('/api/topic03/train-assembly/search', async (req, res) => {
  try {
    const {
      userId,
      taskId,
    ...
    if (userId) searchParams.userId = parseInt(userId);
    if (taskId) searchParams.taskId = parseInt(taskId);
    if (minRate !== undefined) searchParams.minRate = parseFloat(minRate);
    if (maxRate !== undefined) searchParams.maxRate = parseFloat(maxRate);
