...
export default {
  ...
  methods: {
    ...
    async loadFlowData() {
      ...
      try {
        console.log('🔄 PurchaseFlow组件开始从API加载采购流程图数据...');
        ...
        // 调用API服务
        const response = await subProcessDataApi.getMermaidFlowData('purchase');
        ...
        console.log('✅ PurchaseFlow组件采购流程图数据加载成功:', {
        ...
        console.error('❌ PurchaseFlow组件从API加载采购流程图数据失败:', error);
    ...
    addNodeClickEvents() {
      ...
      nodes.forEach(node => {
        ...
        node.addEventListener('click', (event) => {
          ...
          if (flowNode) {
            console.log('PurchaseFlow组件节点被点击:', flowNode);
    ...
    // 解析点击到的Mermaid节点对应的业务节点，避免PU22误识别为PU2
    ...
    async refreshFlowData() {
      console.log('🔄 PurchaseFlow组件刷新数据');
    ...
    async checkApiConnection() {
      try {
        ...
        console.log('🔍 PurchaseFlow API连接检查:', status);
        return status;
      } catch (error) {
        console.error('❌ PurchaseFlow API连接检查失败:', error);
    ...
    getComponentStatus() {
      ...
      console.log('📊 PurchaseFlow组件状态:', status);
      return status;
    }
  },
