<template>
  <div class="marketing-flow-container">
    <!-- 加载状态 -->
...
export default {
  ...
  methods: {
    ...
    async loadFlowData() {
      ...
      try {
        console.log('🔄 MarketingFlow组件开始从API加载营销流程图数据...');
        ...
        // 调用API服务
        const response = await subProcessDataApi.getMermaidFlowData('marketing');
        ...
        console.log('✅ MarketingFlow组件营销流程图数据加载成功:', {
        ...
        console.error('❌ MarketingFlow组件从API加载营销流程图数据失败:', error);
    ...
    async renderMermaid() {
      ...
      try {
        const id = `mermaid-marketing-flow-${Date.now()}`;
    ...
    addNodeClickEvents() {
      ...
      nodes.forEach(node => {
        ...
        node.addEventListener('click', (event) => {
          ...
          if (flowNode) {
            console.log('MarketingFlow组件节点被点击:', flowNode);
    ...
    // 解析点击到的Mermaid节点对应的业务节点，避免MK23误识别为MK2
    ...
    async refreshFlowData() {
      console.log('🔄 MarketingFlow组件刷新数据');
    ...
    async checkApiConnection() {
      try {
        ...
        console.log('🔍 MarketingFlow API连接检查:', status);
        return status;
      } catch (error) {
        console.error('❌ MarketingFlow API连接检查失败:', error);
    ...
    getComponentStatus() {
      ...
      console.log('📊 MarketingFlow组件状态:', status);
      return status;
    }
    },
