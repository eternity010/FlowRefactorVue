<template>
  <div class="sub-process-management">
    ...
    <div v-if="loading || riskDataLoading" class="loading-container">
      ...
      <p v-if="loading && riskDataLoading">正在加载子流程数据和风险数据，请稍候...</p>
      <p v-else-if="loading">正在加载子流程数据，请稍候...</p>
...
export default {
  ...
  computed: {
    ...
    currentRiskData() {
      // 优先使用从数据库获取的风险数据
      if (this.dbRiskData && this.dbRiskData[this.currentProcess]) {
        return this.dbRiskData[this.currentProcess];
      }
    ...
    // 对话框标题
    nodeListDialogTitle() {
      const processName = this.getProcessName(this.currentProcess);
      const riskTypeName = this.getRiskTypeName(this.selectedRiskType);
      return `${processName} - ${riskTypeName}节点列表`;
    }
  },
  watch: {
    // 监听当前流程变化
  ...
  methods: {
    ...
    async loadRiskData() {
      ...
      try {
        console.log('🔄 SubProcessManagement组件开始从MySQL加载风险数据...');
        ...
        if (result.success && result.data) {
          ...
          // 详细显示加载的风险数据
          console.log('✅ SubProcessManagement组件风险数据加载成功:', {
            processTypes: Object.keys(result.data),
            timestamp: new Date().toISOString()
          });
          
          // 显示每个环节的风险数据概况
          Object.entries(result.data).forEach(([processType, riskData]) => {
            console.log(`📊 ${processType} 环节风险数据:`, {
              total: riskData.total,
              high: riskData.highRisk.count,
              medium: riskData.mediumRisk.count,
              normal: riskData.normal.count
            });
          });
        } else {
        ...
        console.error('❌ SubProcessManagement组件从MySQL加载风险数据失败:', error);
    ...
    async handleProcessChange(processKey) {
      ...
      if (!this.processData || !this.processData[processKey]) {
        console.log(`⚠️  ${processKey}流程数据不可用，尝试重新加载...`);
        await this.loadData();
      }
      
      // 检查风险数据是否可用
      if (!this.dbRiskData || !this.dbRiskData[processKey]) {
        console.log(`⚠️  ${processKey}风险数据不可用，尝试重新加载...`);
        await this.loadRiskData();
      }
      
      console.log(`✅ ${processKey}数据切换完成 - 流程数据: ${this.processData && this.processData[processKey] ? '✓' : '✗'}, 风险数据: ${this.dbRiskData && this.dbRiskData[processKey] ? '✓' : '✗'}`);
    },
    ...
    debugRiskData() {
      ...
      if (this.dbRiskData) {
        console.log('📊 数据库风险数据包含的环节:', Object.keys(this.dbRiskData));
        Object.entries(this.dbRiskData).forEach(([processType, data]) => {
          console.log(`${processType}:`, {
            total: data.total,
            high: (data.highRisk && data.highRisk.count) || 0,
            medium: (data.mediumRisk && data.mediumRisk.count) || 0,
            normal: (data.normal && data.normal.count) || 0
          });
        });
      }
    },
     ...
     */
    debugNodeDetails() {
      console.log('🔍 当前环节节点详情:');
      if (this.currentRiskData) {
        console.log(`${this.currentProcess} 环节风险分析:`);
